...
/**
 * GET /api/dashboard/por-entidade
 * Retorna distribuição de comissões por entidade
 * 
 * Lógica híbrida:
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const whereClause = `WHERE ${whereConditions.join(" AND ")} ${filtroPapel}`
    ...
    // LIMIT não aceita placeholder, então inserimos diretamente (já validado como número)
    const safeLimit = Math.max(1, Math.min(limit, 50))

    // Usar lógica híbrida para calcular valor por entidade
    ...
    const resultado = rows.map((row: any) => {
      ...
      const valor = valorBruto - descontoTotal
    ...
    return NextResponse.json(
      { error: error.message || "Erro ao buscar por entidade" },
      { status: 500 }
    )
  } finally {
    if (connection) {
